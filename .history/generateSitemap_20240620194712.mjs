...
async function fetchCMSItems(): Promise<Slug[]> {
    try {
        ...
        const slugs: Slug[] = posts.map((post: any) => ({
...
async function fetchPlasmicPaths(): Promise<Slug[]> {
    try {
        const pageModules = await PLASMIC.fetchPages();
        const paths: Slug[] = pageModules.map((mod: any) => ({
            url: `/${mod.path}`,
            changefreq: 'daily',
            priority: 0.7
        }));
